    ...
    Object.keys(this.malla).forEach((e) => {
      ...
      i.append("title").text("Semestre " + r),
        ...
        Object.keys(this.malla[e]).forEach((s) => {
          this.malla[e][s].draw(o, c, d, this.scaleX, this.scaleY),
            (d += this.subjectType.getDisplayHeight(this.scaleY) + t);
        }),
        (c += this.subjectType.getDisplayWidth(this.scaleX) + t);
    });
  }
...
class Ramo {
  ...
  constructor(e, t, s, a, r = [], i, l, n = 0, o = !1, c = "") {
    (this.name = e),
      (this.sigla = t),
      (this.credits = s),
      (this.category = a),
  ...
  draw(e, t, s, a, r) {
    ...
    this.prer.forEach((e) => {
      let r = 9,
        ...
        o = 5;
      a < 0.83 && (r--, i--, (o = 1));
      let c = this.malla.categories[this.malla.ALLSUBJECTS[e].category][0];
      this.ramo
        .append("circle")
        .attr("cx", t + r + m + o)
        .attr("cy", s + l - n / 2)
...
class Generator extends SemesterManager {
  constructor(e, t) {
    super(e, t),
      (this.mallaEditor = new MallaEditor(
        this,
        "#unoficialSubjects",
        "#sectors"
      ));
  ...
  displaySubject(e) {
    if (e.approved) e.showWarning();
    else {
      let t = this.card.select(".subjects").append("li");
      t.attr("id", "per-" + e.sigla)
        .classed("list-group-item", !0)
        .classed("d-flex", !0)
        .classed("align-items-center", !0)
        .classed("py-0", !0)
        .classed("pr-0", !0)
        .style("opacity", "0.01")
        .transition()
        ...
        .style("opacity", "1");
      let s = t.append("div");
      s.classed("flex-grow-1", !0).classed("mr-3", !0).classed("py-2", !0),
        s.append("p").classed("my-0", !0).text(e.name);
      let a = Boolean(this.semester % 2);
      a && "P" === e.dictatesIn
        ? s
            .append("p")
            .attr("id", "dictatesIn-" + e.sigla)
            .style("line-height", 1)
            .classed("my-0", !0)
            .append("small")
            .classed("text-center my-0 text-danger", !0)
            .text(
        ...
        : a ||
          ...
          s
            ...
            .classed("my-0", !0)
            .append("small")
            .classed("text-center text-danger", !0)
            .text(
              "Esta asignatura normalmente solo se dicta en semestres Impares"
            );
      let r = t.append("button");
      r.classed("btn btn-secondary", !0).attr("type", "button").text("Editar"),
        r.on(
          "click",
          this.mallaEditor.setUpModal.bind(this.mallaEditor, !0, e)
        ),
        (this.displayedSubjects[e.sigla] = t),
        this.mallaEditor.updateState(e),
        this.saveSemesters();
    }
  }
  updateDisplayedSubject(e) {
    super.updateDisplayedSubject(e);
    let t = this.displayedSubjects[e.sigla];
    if (t) {
      t.select("div").text(e.name);
      let s = t.select("#dictatesIn-" + e.sigla);
      s && s.remove();
      let a = Boolean(this.semester % 2);
      a && "P" === e.dictatesIn
        ? t
  ...
  unDisplaySubject(e) {
    this.displayedSubjects[e.sigla]
      .transition()
      .duration(300)
      .style("opacity", "0.001")
      .remove(),
  ...
  nextSemester() {
    super.nextSemester(), this.mallaEditor && this.mallaEditor.semesterChange();
  }
  prevSemester() {
    super.prevSemester(), this.mallaEditor && this.mallaEditor.semesterChange();
  }
  cleanAll() {
    super.cleanAll(),
  ...
  saveSemesters() {
    if (this.saveEnabled) {
      ...
      Object.keys(this.selectedPerSemester).forEach((t) => {
        let s = [];
        this.selectedPerSemester[t].forEach((e) => {
          s.push(e.sigla);
        }),
          (e[t] = s);
      }),
        (e = JSON.stringify(e)),
        (localStorage["generatorUserData" + this.malla.currentMalla] = e);
    }
  }
  loadSemesters() {
    let e = localStorage["generatorUserData" + this.malla.currentMalla];
    if (!e) {
      if (
        ((e = localStorage["Custom-" + this.malla.currentMalla + "_SEMESTRES"]),
        !e)
      )
        return;
      localStorage["generatorUserData" + this.malla.currentMalla] = e;
...
class MallaEditor {
  ...
  displaySubject(e) {
    ...
    n.classList.add("py-0");
    let o = document.createElement("div");
    o.classList.add("btn-group"), o.setAttribute("role", "group");
    let c = document.createElement("button");
    c.setAttribute("id", "sel-" + e.sigla),
      c.setAttribute("type", "button"),
      c.classList.add("btn", "btn-secondary"),
      (c.textContent = "Seleccionar"),
      c.addEventListener("click", (t) => {
        e.selectRamo(), this.updateState(e);
      }),
      e.selected && (c.textContent = "Deseleccionar"),
      o.append(c);
    let d = null,
      h = null;
    if (
  ...
  createAdvancedSubject() {
    ...
    e.querySelector("#prerList")
      .querySelectorAll("li")
      .forEach((e) => {
        n.push(e.getAttribute("id").slice(4));
      });
    let o = new SelectableRamo(
      t,
      s,
      a,
      i,
    ...
    this.subjectList.push(o),
      this.semesterManager.malla.addSubject(o),
      this.createAdvancedSubjectModal.modal("hide"),
      this.displaySubject(o),
      this.saveSubjects();
  }
  editSubject(e) {
  ...
  loadSubjects() {
    ...
    if (
      ...
      Object.keys(t).forEach((e) => {
        let a = s
          .append("li")
          .text(`Ramos que tenían a ${e} como prerrequisito`)
          .append("ul");
        t[e].forEach((e) => {
          a.append("li").text(e);
        });
